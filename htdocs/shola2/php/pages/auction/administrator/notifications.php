	...
	<div id="container">
		<div id="bgwrap">
			...
			<center><h1><br />Welcome Administrator</h1></center>
				...
				$bidnum = mysql_query("SELECT * FROM bid_report LEFT JOIN customer ON customer.user_id = bid_report.bidder LEFT JOIN products ON products.product_id = bid_report.product_id WHERE bid_report.status = 0") or die(mysql_error());
						$count = 0;
				...
				$endedsum = mysql_query("SELECT * FROM products WHERE due_date < '$datenow' AND status = 0") or die(mysql_error());
					$counters = 0;
