...
//most of the code here is the same as the one before and has ben explained there.....
	...
	public function fetchItems($categoryName){
		...
		$sql="SELECT * FROM items WHERE Category=:categoryName";
				...
				//echo "$num_rows Rows\n";
		   			//$dbh = new PDO("mysql:dbname=$dbname", $username, $password);//['$categoryName']
					//$dbh->setAttribute(PDO::ATTR_EMULATE_PREPARES, FALSE);
					try{
						//$sd=SELECT COUNT(*) FROM items WHERE Category= 'Automotives';
                          //echo $db->SELECT COUNT(*) FROM items WHERE Category= "Automotives";
